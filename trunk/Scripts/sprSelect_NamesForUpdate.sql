...
AS
	...
	--names that point to a non existent parent - shouldn't happen, but if it does a refresh should fix it
	insert @names 
	select nameguid
	from tblName n
	where not exists (select nameguid from tblname where nameguid = n.nameparentfk)
