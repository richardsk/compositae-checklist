...
Public Class Utility
    ...
    Public Shared Function GetProviderConflictPrefName(ByVal provData As DataSet, ByVal conceptVal As String) As Boolean
        Dim hasConf As Boolean = False

        If conceptVal IsNot Nothing AndAlso conceptVal.Length > 0 Then
            Dim provs As String = ""
            'check for conflicting data
            For Each pc As DataRow In provData.Tables(0).Rows
                If pc("PCName2").ToString.Length > 0 AndAlso pc("PCName2").ToString <> conceptVal AndAlso pc("PCRRelationship") = "has preferred name" Then
                    hasConf = True
                End If
            Next
        Else
            'get provider values
            For Each pc As DataRow In provData.Tables(0).Rows
                If pc("PCName2").ToString.Length > 0 AndAlso pc("PCRRelationship") = "has preferred name" Then
                    hasConf = True
                End If
            Next
        End If
    ...
    Public Shared Function ProviderNamesHaveConflict(ByVal provData As DataSet, ByVal nameVal As String, ByVal provNameField As String) As Boolean
        Dim hasConf As Boolean = False
        ...
        Else
            'get provider values
            For Each pn As DataRow In provData.Tables(0).Rows
                If pn(provNameField).ToString.Length > 0 Then
                    hasConf = True
                End If
            Next
        End If
