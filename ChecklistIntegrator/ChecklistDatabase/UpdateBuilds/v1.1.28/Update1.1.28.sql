if not exists (select * from sysindexes where name = 'IX_tblName_Basionym')
begin
	CREATE NONCLUSTERED INDEX IX_tblName_Basionym ON dbo.tblName
		(
		NameBasionymFk
		) WITH( STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]

	ALTER TABLE dbo.tblName SET (LOCK_ESCALATION = TABLE)
end
go
