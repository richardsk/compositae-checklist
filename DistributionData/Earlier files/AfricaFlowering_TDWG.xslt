...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="ProcessRegion">
		...
		<xsl:choose>
			<xsl:when test="$vUse='AN'">
				<xsl:call-template name="Northern"/>
			</xsl:when>
			<xsl:when test="$vUse='RN'">
				<xsl:call-template name="Northern"/>
			</xsl:when>
			<xsl:when test="$vUse='AS'">
				<xsl:call-template name="Southern"/>
			</xsl:when>
			<xsl:when test="$vUse='RS'">
				<xsl:call-template name="Southern"/>
			</xsl:when>
			<xsl:when test="$vUse='AT'">
				<xsl:call-template name="Tropical"/>
			</xsl:when>
			<xsl:when test="$vUse='RT'">
				<xsl:call-template name="Tropical"/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="Northern">
		<Distribution schema="TDWG Level 2" region="20" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
	</xsl:template>
	<xsl:template name="Southern">
		<Distribution schema="TDWG Level 2" region="27" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
	</xsl:template>
	<xsl:template name="Tropical">
		<Distribution schema="TDWG Level 2" region="22" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
		<Distribution schema="TDWG Level 2" region="23" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
		<Distribution schema="TDWG Level 2" region="24" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
		<Distribution schema="TDWG Level 2" region="25" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
		<Distribution schema="TDWG Level 2" region="26" occurrence="Present" origin="">
			<xsl:attribute name="isOriginal">false</xsl:attribute>
		</Distribution>
	</xsl:template>
</xsl:stylesheet>
