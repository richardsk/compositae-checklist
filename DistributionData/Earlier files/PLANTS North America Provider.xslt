...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="conRegion">
		<xsl:param name="pRegion"/>
		<xsl:choose>
			<xsl:when test="$pRegion='CAN - Alberta'">US States;CAN - Alberta</xsl:when>
			<xsl:when test="$pRegion='CAN - Manitoba'">US States;CAN - Manitoba</xsl:when>
			<xsl:when test="$pRegion='CAN - Newfoundland'">US States;CAN - Newfoundland</xsl:when>
			<xsl:when test="$pRegion='CAN - Northwest Territory'">US States;CAN - Northwest Territory</xsl:when>
			<xsl:when test="$pRegion='CAN - Nova Scotia'">US States;CAN - Nova Scotia</xsl:when>
			<xsl:when test="$pRegion='CAN - Nunavut'">US States;CAN - Nunavut</xsl:when>
			<xsl:when test="$pRegion='CAN - Ontario'">US States;CAN - Ontario</xsl:when>
			<xsl:when test="$pRegion='CAN - Prince Edward Island'">US States;CAN - Prince Edward Island</xsl:when>
			<xsl:when test="$pRegion='CAN - Quebec'">US States;CAN - Quebec</xsl:when>
			<xsl:when test="$pRegion='PR - Puerto Rico'">US States;PR - Puerto Rico</xsl:when>
			...
			<xsl:when test="$pRegion='CAN - Alberta'">US States;CAN - Alberta</xsl:when>
			<xsl:when test="$pRegion='CAN - British Columbia'">US States;CAN - British Columbia</xsl:when>
			<xsl:when test="$pRegion='CAN - Labrador'">US States;CAN - Labrador</xsl:when>
			<xsl:when test="$pRegion='CAN - New Brunswick'">US States;CAN - New Brunswick</xsl:when>
			<xsl:when test="$pRegion='CAN - Saskatchewan'">US States;CAN - Saskatchewan</xsl:when>
			<xsl:when test="$pRegion='CAN - Yukon Territory'">US States;CAN - Yukon Territory</xsl:when>
			<xsl:when test="$pRegion='GR - Greenland'">US States;GR - Greenland</xsl:when>
			<xsl:when test="$pRegion='SPM - Saint Pierre and Miquelon'">US States;SPM - Saint Pierre and Miquelon</xsl:when>
			...
			<xsl:when test="$pRegion='VI - Virgin Islands'">US States;VI - Virgin Islands</xsl:when>
