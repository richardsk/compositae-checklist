...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="conRegion">
		<xsl:param name="pRegion"/>
		<xsl:choose>
			<xsl:when test="$pRegion='la'">Caribbean;Lesser Antilles</xsl:when>
			<xsl:when test="$pRegion='ja'">Caribbean;Jamaica</xsl:when>
			<xsl:when test="$pRegion='cu'">Caribbean;Cuba</xsl:when>
			<xsl:when test="$pRegion='ci'">Caribbean;Cayman Islands</xsl:when>
			<xsl:when test="$pRegion='hi'">Caribbean;Hispaniola</xsl:when>
			<xsl:when test="$pRegion='pr'">Caribbean;Puerto Rico</xsl:when>
			<xsl:when test="$pRegion='vi'">Caribbean;Virgin Islands</xsl:when>
			<xsl:when test="$pRegion='ba'">Caribbean;Bahamas</xsl:when>
			<xsl:when test="$pRegion='ma'">Continent;Northern America</xsl:when>
			<xsl:when test="$pRegion='mex'">ISO Countries;Mexico</xsl:when>
			<xsl:when test="$pRegion='ca'">Region;Central America</xsl:when>
			<xsl:when test="$pRegion='sa'">Continent;South America</xsl:when>
			<xsl:when test="$pRegion='wi'">Caribbean;West Indies</xsl:when>
		</xsl:choose>
	</xsl:template>
